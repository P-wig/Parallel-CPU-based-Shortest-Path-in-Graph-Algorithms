...
The $\delta$ parameter was set to ten times the average edge weight of the input graph ($\delta = 10 \times \text{avg. edge weight}$) for all experiments, as this value provided a good balance between parallelism and synchronization overhead in practice.

All sequential implementations read the input graph in DIMACS format, computed shortest path distances from a designated source node (node 0), and wrote the results to a standardized output file. These baseline results were used to validate the correctness of the parallel implementations and to provide a point of comparison for evaluating speedup and scalability.
